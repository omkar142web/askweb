...
function buildTextPayload(question, escapeFn) {
    const commandBlocks = (question.commandResults || [])
        .map((r) => formatCommandResult(r, escapeFn))
        .join("\n\n");
    const parts = [];
...
function buildFullPrompt(question, options = {}, escapeFn) {
...
function formatCommandResult(result, escapeFn) {
    const e = escapeFn || ((s) => s);
    ...
    let block = `<command name="${e(result.command)}">\n`;
