...
// Provider-capped variant: like buildDeliveryPlan, but chunk bodies never
// exceed maxPartChars (tags/header excluded from the cap budget via the
// caller's margin). Needed by providers whose composer truncates input
// below the shared ANON_PART_SIZE_CEILING packing (e.g. Gemini caps at
// ~32K chars). An explicit ASKWEB_CHUNK_SIZE override still wins.
function buildCappedTransmissionPlan(payload, maxPartChars, manualChunkSize = null) {
    if (manualChunkSize) return buildTransmissionPlan(payload, manualChunkSize);
    const autoChunk = Math.ceil(payload.length / planTransmissionParts(payload.length));
    return buildTransmissionPlan(payload, Math.min(autoChunk, maxPartChars));
}
